...
func TestSliceDividesAndRangesItsCount(t *testing.T) {
	...
	for _, tc := range []struct{ src, want string }{
		...
		{`{{ [1,2,3]|slice(0)|list }}`, wantZeroDivision("integer division or modulo by zero")},
		{`{{ [1,2,3]|slice(false)|list }}`, wantZeroDivision("integer division or modulo by zero")},
		...
		{`{{ []|slice("x")|list }}`,
			"unsupported operand type(s) for //: 'int' and 'str'"},
		{`{{ []|slice(0)|list }}`, wantZeroDivision("integer division or modulo by zero")},
	} {
		tmpl, err := env.FromString(tc.src)
		if err != nil {
			t.Errorf("%s: %v", tc.src, err)
