...
// 3.11 to 3.14, 38 of gojja2's 2,197 corpus cases answer differently.
//
...
// The interpreters gojja2 reproduces. [DefaultPythonVersion] is the newest.
const (
	...
	// gets: the newest, because a template author reaching for jinja2
	// today runs it on a current interpreter.
...
//
// The default is the newest. Choose an older one to match a service already
// running on it:
//
//	env, err := gojja2.New(gojja2.WithPythonVersion(gojja2.Python311))
//
// What it changes is a closed list -- thirteen rules, each named in
	...
	return func(e *Environment) error {
		if !v.Known() {
			return fmt.Errorf("gojja2: unknown PythonVersion %d; gojja2 reproduces %s to %s",
				int(v), value.Python311, value.DefaultPythonVersion)
		}
		e.pyVersion = v
		return nil
	}
