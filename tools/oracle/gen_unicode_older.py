...
"""Write where an older CPython's Unicode answers differ from the default's.

CPython carries its own Unicode, so the interpreter decides a template's case
mappings, which characters are digits, and how repr escapes them. Between 3.11
and 3.14 that is 10,311 code points -- and until now gojja2 answered the pinned
version's way whatever WithPythonVersion asked for, which made the option mean
less than it said.

Almost all of it is one property. 3.11 and 3.14 disagree about isprintable for
10,294 code points and about everything else put together for 257, because the
difference is simply which characters had been assigned yet -- and newly
assigned characters arrive in blocks, so 10,294 of them collapse to 75 ranges.
The whole table for all three older versions is about 6 KB.
...
DST = ROOT / "value" / "unicode_older.go"
DUMPER = Path(__file__).resolve().parent / "dump_unicode.py"

# The interpreters gojja2 reproduces, oldest first. The last is the default and
# is the one the committed tables are built for, so it needs no overrides.
...
def go_answers() -> dict[int, tuple[bool, bool, bool]]:
    ...
    the pinned CPython agree about which characters exist -- and they do not.
    Go 1.26 is Unicode 15.0 and CPython 3.14 is 16.0, so Garay is a letter to
...
''']
    ...
    for v in VERSIONS:
        ...
        parts.append(range_table(
            f"printableOlder{suffix}", printable,
            f"// printableOlder{suffix} is where CPython {v} and the default disagree about\n"
        ...
        for col, nm in ((UPPER, "Upper"), (LOWER, "Lower"), (TITLE, "Title"), (FOLD, "Fold")):
            parts.append(go_rune_map(
                f"{nm.lower()}Older{suffix}", maps[col],
                f"// {nm.lower()}Older{suffix} is str.{nm.lower()}() where CPython {v} differs\n"
                f"// from the default. {len(maps[col])} code points."))
        for pts, nm in ((digit, "digit"), (numeric, "numeric"), (decimal, "decimal"),
                        (lower, "isLower"), (upper, "isUpper"), (title, "isTitle"),
                        (alpha, "isAlpha")):
            parts.append(range_table(
                f"{nm}Older{suffix}", pts,
                f"// {nm}Older{suffix} is where CPython {v} and the default disagree\n"
    ...
    for v in VERSIONS:
        ...
        rows.append(
            f"\tPython{suffix}: {{\n"
            f"\t\tprintable: printableOlder{suffix},\n"
            f"\t\tupper: upperOlder{suffix}, lower: lowerOlder{suffix},\n"
            f"\t\ttitle: titleOlder{suffix}, fold: foldOlder{suffix},\n"
            f"\t\tdigit: digitOlder{suffix}, numeric: numericOlder{suffix},\n"
            f"\t\tdecimal: decimalOlder{suffix},\n"
            f"\t\tisLower: isLowerOlder{suffix}, isUpper: isUpperOlder{suffix},\n"
            f"\t\tisTitle: isTitleOlder{suffix}, isAlpha: isAlphaOlder{suffix},\n"
            f"\t}},\n")
    parts.append(
    ...
    sys.stderr.write(f"default vs Go: {len(alphaFix)} isalpha, {len(printableFix)} isprintable\n")
