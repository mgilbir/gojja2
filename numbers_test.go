...
func TestRoundIsPythonsRound(t *testing.T) {
	...
	for _, tc := range []struct{ src, want string }{
		...
		{`{{ 1|round(method=[]) }}`, wantUnhashable("list", "list", asSetElement)},
		{`{{ 1.5|round(0, {}) }}`, wantUnhashable("dict", "dict", asSetElement)},
		...
		{`{{ 1.5|round(-400, "ceil") }}`, wantZeroDivision("float division by zero")},
	} {
		tmpl, err := env.FromString(tc.src)
		if err != nil {
			t.Fatalf("compile %q: %v", tc.src, err)
